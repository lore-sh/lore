...
describe("studio api", () => {
  testWithTmp("list tables and read table data with sort/filter/pagination", async () => {
    ...
    await applyPlan(insertOnePath, { dbPath });
    await applyPlan(insertTwoPath, { dbPath });

    const tables = listStudioTables({ dbPath });
  ...
  testWithTmp("schema view includes column constraints", async () => {
    ...
    const schema = getStudioSchema({ dbPath });
  ...
  testWithTmp("history detail returns operations and effects", async () => {
    ...
    const history = listStudioHistory({ dbPath });
    ...
    const detail = getStudioCommitDetail(commitId ?? "", { dbPath });
  ...
  testWithTmp("history list applies limit without decoding skipped commit operations", async () => {
    ...
    const history = listStudioHistory({ dbPath, limit: 1 });
  ...
  testWithTmp("invalid filter column fails clearly", async () => {
    ...
    try {
      readStudioTable({ dbPath, table: "expenses", filters: { unknown: "1" } });
