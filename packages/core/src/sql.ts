...
): { ok: true; whereClause: string } | { ok: false; message: string } {
  const keys = Object.keys(pk).sort((a, b) => a.localeCompare(b));
  if (keys.length === 0) {
    return { ok: false, message: "Primary key predicate must not be empty" };
  }
  const clauses: string[] = [];
  for (const key of keys) {
    const literal = pk[key];
    if (!literal) {
      return { ok: false, message: `Primary key literal is missing for column ${key}` };
    }
    const quoted = quoteIdentifier(key, { unsafe: true });
    clauses.push(literal.toUpperCase() === "NULL" ? `${quoted} IS NULL` : `${quoted} = ${literal}`);
  }
  return { ok: true, whereClause: clauses.join(" AND ") };
}
