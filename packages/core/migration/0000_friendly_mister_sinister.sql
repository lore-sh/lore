...
CREATE TABLE `_toss_commit` (
	...
	FOREIGN KEY (`revert_target_id`) REFERENCES `_toss_commit`(`commit_id`) ON UPDATE no action ON DELETE set null,
	...
	CONSTRAINT "chk_toss_commit_revertible_bool" CHECK("_toss_commit"."revertible" in (0, 1)),
	...
	CONSTRAINT "chk_toss_commit_revert_target_hash_len_or_null" CHECK("_toss_commit"."revert_target_id" is null or length("_toss_commit"."revert_target_id") = 64)
);
--> statement-breakpoint
CREATE UNIQUE INDEX `_toss_commit_seq_unique` ON `_toss_commit` (`seq`);--> statement-breakpoint
CREATE TABLE `_toss_meta` (
	`key` text PRIMARY KEY NOT NULL,
	`value` text NOT NULL,
	CONSTRAINT "chk_toss_meta_key_non_empty" CHECK(length("_toss_meta"."key") > 0)
);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE INDEX `idx_toss_reflog_ref_name_id` ON `_toss_reflog` (`ref_name`,`id`);--> statement-breakpoint
CREATE TABLE `_toss_row_effect` (
	`commit_id` text NOT NULL,
	...
	FOREIGN KEY (`commit_id`) REFERENCES `_toss_commit`(`commit_id`) ON UPDATE no action ON DELETE cascade,
	CONSTRAINT "chk_toss_row_effect_effect_index_non_negative" CHECK("_toss_row_effect"."effect_index" >= 0),
	CONSTRAINT "chk_toss_row_effect_table_name_non_empty" CHECK(length(trim("_toss_row_effect"."table_name")) > 0),
	CONSTRAINT "chk_toss_row_effect_pk_json_valid" CHECK(json_valid("_toss_row_effect"."pk_json")),
	CONSTRAINT "chk_toss_row_effect_before_json_valid_or_null" CHECK("_toss_row_effect"."before_json" is null or json_valid("_toss_row_effect"."before_json")),
	CONSTRAINT "chk_toss_row_effect_after_json_valid_or_null" CHECK("_toss_row_effect"."after_json" is null or json_valid("_toss_row_effect"."after_json")),
	CONSTRAINT "chk_toss_row_effect_before_hash_len_or_null" CHECK("_toss_row_effect"."before_hash" is null or length("_toss_row_effect"."before_hash") = 64),
	CONSTRAINT "chk_toss_row_effect_after_hash_len_or_null" CHECK("_toss_row_effect"."after_hash" is null or length("_toss_row_effect"."after_hash") = 64),
	CONSTRAINT "chk_toss_row_effect_before_hash_pairing" CHECK(("_toss_row_effect"."before_hash" is null) = ("_toss_row_effect"."before_json" is null)),
	CONSTRAINT "chk_toss_row_effect_after_hash_pairing" CHECK(("_toss_row_effect"."after_hash" is null) = ("_toss_row_effect"."after_json" is null)),
	CONSTRAINT "chk_toss_row_effect_op_shape" CHECK(("_toss_row_effect"."op_kind" = 'insert' and "_toss_row_effect"."before_json" is null and "_toss_row_effect"."after_json" is not null)
        or ("_toss_row_effect"."op_kind" = 'update' and "_toss_row_effect"."before_json" is not null and "_toss_row_effect"."after_json" is not null)
        or ("_toss_row_effect"."op_kind" = 'delete' and "_toss_row_effect"."before_json" is not null and "_toss_row_effect"."after_json" is null))
);
--> statement-breakpoint
CREATE INDEX `idx_toss_row_effect_table_pk` ON `_toss_row_effect` (`table_name`,`pk_json`);--> statement-breakpoint
CREATE TABLE `_toss_schema_effect` (
	`commit_id` text NOT NULL,
	`effect_index` integer NOT NULL,
	`table_name` text NOT NULL,
	`before_json` text,
	...
	PRIMARY KEY(`commit_id`, `effect_index`),
	FOREIGN KEY (`commit_id`) REFERENCES `_toss_commit`(`commit_id`) ON UPDATE no action ON DELETE cascade,
	CONSTRAINT "chk_toss_schema_effect_effect_index_non_negative" CHECK("_toss_schema_effect"."effect_index" >= 0),
	CONSTRAINT "chk_toss_schema_effect_table_name_non_empty" CHECK(length(trim("_toss_schema_effect"."table_name")) > 0),
	CONSTRAINT "chk_toss_schema_effect_before_json_valid_or_null" CHECK("_toss_schema_effect"."before_json" is null or json_valid("_toss_schema_effect"."before_json")),
	CONSTRAINT "chk_toss_schema_effect_after_json_valid_or_null" CHECK("_toss_schema_effect"."after_json" is null or json_valid("_toss_schema_effect"."after_json")),
	CONSTRAINT "chk_toss_schema_effect_some_side_exists" CHECK("_toss_schema_effect"."before_json" is not null or "_toss_schema_effect"."after_json" is not null)
);
--> statement-breakpoint
CREATE INDEX `idx_toss_schema_effect_table_name` ON `_toss_schema_effect` (`table_name`);--> statement-breakpoint
